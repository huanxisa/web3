...
func handleTransfer(ctx context.Context, client *ethclient.Client, parsedABI abi.ABI, contractHex, toHex, amountStr string) {
	...
	// ─── 费用相关概念梳理（EIP-1559）────────────────────────────────────────────
	// Gas：执行交易消耗的「计算量」单位。每笔交易设 Gas Limit = 最多愿意消耗多少单位。
	// 实际每单位 Gas 的单价由链/区块决定，用户通过 Gas Fee Cap / Tip 表达意愿。
	//
	// 对比（旧版 vs EIP-1559）：
	//   旧版：只有 Gas Price（wei/gas），用户出价，全给矿工。
	//   EIP-1559：单价拆成 Base Fee + Tip。Base Fee 由协议按拥堵算，销毁；Tip 给验证者。
	//
	// 关键字段含义：
	//   Gas Limit：本笔交易最多用多少 Gas，用超会 revert，未用完会退回。
	//   Base Fee：当前区块的「基础费」（wei/gas），由链定，被销毁。
	//   Gas Tip Cap：你愿意给验证者的「小费」上限（wei/gas），决定打包优先级。
	//   Gas Fee Cap：你愿意为每单位 Gas 付的「总价上限」= base + tip 的上限，用于防拥堵时被宰。
	//
	// 为什么还要 Gas Fee Cap？（base fee + tip 不是已经决定单价了吗？）
	//   Base Fee 是「下一个块」的，发交易时还不知道；拥堵时下一块 base fee 会涨。
	//   没有上限就等于说「链收多少我都认」，可能被收得很贵。Gas Fee Cap 是单价天花板：
	//   实际收取的单价 = min(gasFeeCap, 实际块的 baseFee + tip)，避免 base fee 暴涨时多付。
	//
	// 关系与约束：
	//   实际单价 = min(gasFeeCap, baseFee + tip)，且 tip 部分给验证者。
	//   总 Gas 费（最坏）≤ gasFeeCap × gasLimit，故用 gasFeeCap×gasLimit 做余额检查。
	// ─────────────────────────────────────────────────────────────────────────
	...
	// 增加 20% 的缓冲，避免执行环境差异导致 Gas 不足而 revert
	gasLimit = gasLimit * 120 / 100

	// 获取建议的 Gas Tip（小费），用于 EIP-1559
	gasTipCap, err := client.SuggestGasTipCap(ctx)
	if err != nil {
		log.Fatalf("failed to get gas tip cap: %v", err)
	}

	// 获取当前区块 Base Fee，用于计算 Gas Fee Cap
	...
	if baseFee == nil {
		// 链不支持 EIP-1559 时退化为传统 Gas Price
	...
	// Gas Fee Cap = 预估下一块 base 的 2 倍 + tip，保证能进块且不超付
	gasFeeCap := new(big.Int).Add(
		new(big.Int).Mul(baseFee, big.NewInt(2)),
		gasTipCap,
	)

	// 检查 ETH 余额是否足够支付 Gas（ERC-20 不转 ETH，只需付 Gas）
	balance, err := client.BalanceAt(ctx, fromAddr, nil)
	if err != nil {
		log.Fatalf("failed to get balance: %v", err)
	}

	// 总 Gas 费上限 = gasFeeCap × gasLimit，用于保守的余额校验
	...
	// 构造 EIP-1559 动态费用交易（ERC-20 transfer 的 value=0，资金在 Data 里）
