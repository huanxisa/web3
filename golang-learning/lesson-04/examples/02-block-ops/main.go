...
func fetchBlockWithRetry(ctx context.Context, client *ethclient.Client, blockNumber *big.Int, maxRetries int) (*types.Block, error) {
	...
	for i := 0; i < maxRetries; i++ {
		// 每次重试使用新的子上下文：单次请求 10s 超时，且重试时获得新的 10s 窗口（不继承上次的“已取消”状态）
		reqCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
		block, err := client.BlockByNumber(reqCtx, blockNumber)
		cancel() // 立即释放与 reqCtx 关联的定时器；循环内不用 defer，否则要等函数返回才执行，会堆积资源
...
// fetchBlockRange 批量查询区块范围，带速率限制（单位时间内最多执行 N 次调用）
func fetchBlockRange(ctx context.Context, client *ethclient.Client, start, end uint64, interval time.Duration) {
	fmt.Printf("\n=== Fetching Block Range [%d, %d] ===\n", start, end)
	fmt.Printf("Rate Limit: at most 1 call per %v\n\n", interval)

	successCount := 0
	skipCount := 0
	// 限速器：每 interval 补充 1 个令牌，桶容量 1，即「单位时间内最多 1 次调用」
	limiter := rate.NewLimiter(rate.Every(interval), 1)

	for num := start; num <= end; num++ {
		// 在单位时间内限制调用次数：无令牌时阻塞直到获得令牌
		if err := limiter.Wait(ctx); err != nil {
			log.Printf("[WARN] rate limiter: %v", err)
			return
		}
