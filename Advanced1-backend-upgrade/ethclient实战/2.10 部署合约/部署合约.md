...
编译合约，生成二进制字节码文件 `store_sol_Store.bin`：

```bash
solcjs --bin Store.sol
```

生成合约 ABI 文件 `store_sol_Store.abi`：
...
在项目根目录（有 `foundry.toml` 或 `src/` 的目录）执行编译：

```bash
forge build
```

从编译产物中导出 ABI 和 bytecode（合约路径以实际为准，例如 `src/Store.sol:Store`）：

```bash
# 导出 ABI
forge inspect src/Store.sol:Store abi > Store_sol_Store.abi

# 导出部署用字节码（含构造函数）
forge inspect src/Store.sol:Store bytecode > Store_sol_Store.bin
```

若合约在项目根目录的 `Store.sol`，则使用：

```bash
forge inspect Store.sol:Store abi > Store_sol_Store.abi
forge inspect Store.sol:Store bytecode > Store_sol_Store.bin
```
...
使用 abigen 根据 bin 和 abi 生成 Go 代码（Node 方式会得到 `Store_sol_Store.bin` / `Store_sol_Store.abi`；Foundry 方式需将 `forge inspect` 输出重定向为同名文件以便与下面命令一致）：
